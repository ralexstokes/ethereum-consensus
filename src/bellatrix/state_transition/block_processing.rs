...
use spec::{
    ...
    BeaconBlockHeader, BeaconState, Deposit, DepositMessage, ParticipationFlags, ProposerSlashing,
    SignedVoluntaryExit, SyncAggregate, Validator, DEPOSIT_CONTRACT_TREE_DEPTH,
        ...
        for (flag_index, weight) in PARTICIPATION_FLAG_WEIGHTS.iter().enumerate() {
                ...
                && !has_flag(epoch_participation[index], flag_index as u8)
            {
                ...
                proposer_reward_numerator +=
                    crate::bellatrix::get_base_reward(state, index, context)? * weight;
            }
        }
    }
