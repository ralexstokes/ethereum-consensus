use crate::{
    ...
    electra::{
        ...
        compute_epoch_at_slot, compute_exit_epoch_and_update_churn, compute_timestamp_at_slot,
        ...
        BeaconState, BlsPublicKey, BlsSignature, Bytes32, ConsolidationRequest, DepositMessage,
        DepositReceipt, DomainType, ExecutionAddress, ExecutionLayerWithdrawalRequest, Gwei,
        InvalidAttestation, InvalidDeposit, InvalidExecutionPayload, InvalidOperation,
        InvalidVoluntaryExit, InvalidWithdrawals, NewPayloadRequest, ParticipationFlags,
        PendingBalanceDeposit, PendingConsolidation, PendingPartialWithdrawal, SignedVoluntaryExit,
...
>(
    ...
    if let Some(source_index) =
        is_valid_switch_to_compounding_request(state, consolidation_request, context)
    {
        switch_to_compounding_validator(state, source_index, context)?;
        return Ok(())
    }
    ...
    if state.pending_consolidations.len() == PENDING_CONSOLIDATIONS_LIMIT {
        return Ok(())
    }
    // If there is too little available consolidation churn limit, consolidation requests are
    // ignored
    if get_consolidation_churn_limit(state, context)? <= context.min_activation_balance {
        return Ok(())
    }

    // Verify pubkeys exists
    let (source_index, source_validator) = if let Some((i, v)) = state
        .validators
        .iter()
        .enumerate()
        .find(|(_, v)| v.public_key == consolidation_request.source_public_key)
    {
        (i, v)
    } else {
        return Ok(());
    ...
    let (target_index, target_validator) = if let Some((i, v)) = state
        .validators
        .iter()
        .enumerate()
        .find(|(_, v)| v.public_key == consolidation_request.target_public_key)
    {
        (i, v)
    } else {
        return Ok(());
    ...
    if current_epoch < source_validator.activation_epoch + context.shard_committee_period {
        return Ok(())
    }
    ...
    let source_validator = state.validators.get_mut(source_index).unwrap();
    ...
    state.pending_consolidations.push(PendingConsolidation { source_index, target_index });
...
>(
    ...
    let (index, source_validator) = if let Some((index, validator)) = state
        .validators
        .iter()
        .enumerate()
        .find(|(_, v)| v.public_key == consolidation_request.source_public_key)
